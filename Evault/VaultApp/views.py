...
one={
    'from':"0x41cDEF827fe4bE9Eb7D08cA277F9C149a0902c30"
}
def getContract():
    ...
    deployed_contract_address = '0x28A3B2ad0c562B8A21f5df88FeFCc78981D9bFEB' #contract address
...
def readDoc():
    ...
    document_list = []
    count = contract.functions.getDocumentCount().call(one)
    for i in range(0, count):
        doc_name = contract.functions.getName(i).call(one)
        doc_type = contract.functions.getType(i).call(one)
        desc = contract.functions.getDesc(i).call(one)
        belongs = contract.functions.getBelongs(i).call(one)
        address = contract.functions.getAddress(i).call(one)
        phone = contract.functions.getPhone(i).call(one)
        criminal = contract.functions.getCriminalRecord(i).call(one)
        uid = contract.functions.getUid(i).call(one)
        today_date = contract.functions.getDate(i).call(one)
        file = contract.functions.getFile(i).call(one)
...
def AddDocumentAction(request):
    if request.method == 'POST':
        ...
        msg = contract.functions.saveDocument(document, doc_type, desc, name, address, phone, criminal, uid, today, filename).transact(one)
        context= {'data':'Document Details Added to Blockchain with below Transaction Details<br/><br/>'}
